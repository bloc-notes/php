<?php
   ...
   */
      ...
      */
      function insereEnregistrement($strNomTable) {
        ...
        for ($i = 1; $i <= $NbParametre - 1; $i++) {
            ...
            if(gettype($strParametre) == "string" && mb_substr_count($strParametre,"'") > 0){
   ...
   function selectQuery($sql)
   {
        ...
        if ($result = $this->_cBD->query($sql)) 
        {
            $this->_OK = true;
            $this->_nbEnregistrements = mysqli_num_rows($result);
            $this->_resultsSQL = $result; 
            $this->_listeEnregistrements = $result; 
        }
        else
        {
            $this->_OK = false;
