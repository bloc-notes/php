...
//date valide https://stackoverflow.com/questions/13194322/php-regex-to-check-date-is-in-yyyy-mm-dd-format
// (15-02-2018) deux sens
function dateValideCompletTempo($strDate){
    $booValide = false;
    
    if(substr($strDate, 2, 1) == "-"){
        list($intJour,$intMois,$intAnnee) = array_pad(explode('-', $strDate), 3, 0);
        //echo $intJour ."@".$intMois."@" . $intAnnee;
        $booValide = (ctype_digit("$intJour$intMois$intAnnee") && checkdate($intMois, $intJour, $intAnnee));
        
    }
    else{
        list($intAnnee,$intMois,$intJour) = array_pad(explode('-', $strDate), 3, 0);
        //echo $intJour ."@".$intMois."@" . $intAnnee;
        $booValide = (ctype_digit("$intAnnee$intMois$intJour") && checkdate($intMois, $intJour, $intAnnee));
    }
