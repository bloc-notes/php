    ...
    */
    class mysql {
        ...
        */
        ...
        |----------------------------------------------------------------------------------|
        */
        function __construct($strNomBD, $strNomFichierInfosSensibles, $binArret = false) {
            $this->nomBD = $strNomBD;
            $this->nomFichierInfosSensibles = $strNomFichierInfosSensibles;

            if ($this->connexion($binArret)) {
                $this->selectionneBD();
            }
        }
        /*
	...
	/*
       ...
       */
        ...
        /*----------------------------------------------------------
         * retourneDernierNo (30-04-2018)
         * 
         * Retour: le champs désiré (int) de la dernière ligne de la table
         * ---------------------------------------------------------
         */
        function retourneDernierNo($strNomTable, $strNomChamp) {
            $BDTempo = new mysql($this->nomBD, $this->nomFichierInfosSensibles);
            $intValeurRetournee = 0;
            $BDTempo->requete = "SELECT $strNomChamp FROM $strNomTable ORDER BY " .
                    "$strNomChamp DESC LIMIT 1";
            $BDTempo->listeEnregistrements = mysqli_query($BDTempo->cBD, $BDTempo->requete);
            
            if (mysqli_num_rows($BDTempo->listeEnregistrements) > 0) {
                $intValeurRetournee = $BDTempo->contenuChamp(0, $strNomChamp);
            }
            $BDTempo->deconnexion();
            return $intValeurRetournee;
        }
}
